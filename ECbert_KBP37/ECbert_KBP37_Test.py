...
def eval():
    ...
    with torch.no_grad():
        for index, (val_text, val_label) in enumerate(val_loader):
            ...
            x_token.to(device)
            input_masks = torch.tensor(np.array(input_masks))
            input_masks = input_masks.to(device)

            out = model(x_token, input_masks, x_mark_index_all, device)
        ...
        print("Test: ACC: {}, F1: {}".format(val_acc, f1_value))
